"""
Alkami Technology — MeetMagus Seed Script
    ...
    with Session(engine) as session:
        ...
        session.execute(sql("""
            INSERT INTO tenant_1.bankers
                (name, email, firm_name, title, tenant_id, created_at, updated_at)
            VALUES
            ('David Handler', 'dhandler@tidalpartners.com', 'Tidal Partners',
             'Co-Founder & Managing Partner', 'tenant_1', now(), now())
        ...
        session.execute(sql("""
            ...
            WHERE b.name = 'David Handler' AND p.full_name = 'Jeff Fox'
            ON CONFLICT DO NOTHING;
        """))
        ...
        session.execute(sql("""
            ...
            ((SELECT id FROM tenant_1.bankers WHERE name = 'David Handler'),
        ...
        session.execute(sql("""
            INSERT INTO global.obs_investor
                (company_id, investor_name, stake_pct,
                 filing_type, is_activist, signal_type, status,
                 headline, detail, confidence, source_id,
                 observed_at, created_at)
            SELECT c.id,
                'Jana Partners', 7.9,
                'SC 13D', true, 'ownership_change', 'active',
                'Jana Partners files 13D on Alkami — pushing for strategic sale (7.9% stake)',
            ...
            INSERT INTO global.obs_investor
                (company_id, investor_name, stake_pct,
                 ...
                 observed_at, created_at)
            SELECT c.id,
                'General Atlantic', 18.0,
                '13F', false, 'ownership_change', 'active',
                'General Atlantic holds 18% of Alkami — largest single institutional shareholder',
        ...
        session.execute(sql("""
            INSERT INTO global.obs_financial
                (company_id, signal_type, status, headline, detail,
                 ...
                 confidence, source_id, observed_at, created_at)
            SELECT c.id,
                'revenue_signal', 'active',
                'Alkami FY2025: $444M revenue (+33%), $480M ARR, EBITDA positive',
            ...
            SELECT c.id,
                'revenue_signal', 'active',
                'Alkami Q1 2026: $126M revenue (+29%), $494M ARR, $100M buyback announced',
        ...
        session.execute(sql("""
            ...
            SELECT c.id,
                'acquisition', 'active',
                'Alkami acquires MANTL for $400M — extends into account opening and onboarding',
                ...
                0.99, 'SEC_8K',
                '2025-02-27', now(),
                '{"counterparty_name": "MANTL", "deal_value_usd": 400000000}'::jsonb
            FROM global.companies c WHERE c.ticker = 'ALKT'
            ON CONFLICT DO NOTHING;
            ...
            SELECT c.id,
                ...
                'Alkami gaining share vs Fiserv/FIS — 301 FI clients, 35% ARR growth',
        ...
        session.execute(sql("""
            ...
            SELECT c.id,
                'org_change', 'active',
                'Alkami actively hiring in engineering, product, GTM — no layoff signals',
        ...
        session.execute(sql("""
            ...
            SELECT c.id,
                'sector_trend', 'active',
                'Fintech digital banking consolidation accelerating — PE appetite strong for ARR-rich SaaS',
        ...
        session.execute(sql("""
            ...
            WHERE b.name = 'David Handler' AND c.ticker = 'ALKT'
            ON CONFLICT DO NOTHING;
        """))
        ...
        session.execute(sql("""
            INSERT INTO tenant_1.alerts
                (banker_id, trigger_type, title, body,
                 cited_sources, target_company_id, relevance_score, status, created_at)
            SELECT b.id,
                'activist_13d',
